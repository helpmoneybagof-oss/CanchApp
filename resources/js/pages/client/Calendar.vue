...
// Al cambiar de cancha: limpiar slots y recargar
...
async function reserveSlot() {
    ...
    try {
        await axios.post('/cart/slots', {
            slot_ids: [clickedSlot.value.id],
        });
        router.visit('/checkout');
    } catch (e: any) {
        toast.error(e?.response?.data?.message ?? 'Error al reservar el horario.', 'Error');
...
<template>
    ...
    <AppClientLayout>
        <div class="p-3 lg:p-6 overflow-x-hidden">
            ...
            <!-- Selector de cancha (desplegable) + miniatura clickeable -->
            <div v-if="courts.length >= 1" class="mb-3 flex items-stretch gap-2">
                ...
                <!-- Miniatura: si hay foto, muestra la imagen; si no, ícono. Click → modal grande. -->
                <button
                    v-if="selectedCourt"
                    @click="showPhotoModal = true"
                    type="button"
                    :title="selectedCourt.image_url ? 'Ver foto de la cancha' : 'Esta cancha no tiene foto'"
                    :disabled="!selectedCourt.image_url"
                    class="flex h-auto w-12 shrink-0 items-center justify-center overflow-hidden rounded-2xl border border-border bg-card shadow-sm transition hover:ring-2 hover:ring-primary/40 disabled:cursor-default disabled:opacity-60 disabled:hover:ring-0"
                >
                    <img v-if="selectedCourt.image_url" :src="selectedCourt.image_url" :alt="selectedCourt.name"
                        class="h-full w-full object-cover" />
                    <ImageIcon v-else class="h-5 w-5 text-muted-foreground/60" />
                </button>
            </div>
        ...
        <Teleport to="body">
            <div v-if="showModal && clickedSlot"
                ...
                <div class="w-full max-w-sm rounded-t-3xl bg-background p-6 shadow-xl lg:rounded-3xl">
                    ...
                    <button @click="reserveSlot" :disabled="reserving"
                        class="w-full rounded-2xl bg-primary py-3 text-base font-bold text-primary-foreground shadow transition hover:bg-primary/90 active:scale-95 disabled:opacity-60">
                        {{ !isAuth ? 'Iniciar sesión para reservar' : reserving ? 'Reservando...' : 'Reservar' }}
                    </button>
                </div>
            </div>
        </Teleport>

        <!-- Modal: foto en grande de la cancha seleccionada -->
        <Teleport to="body">
            <div v-if="showPhotoModal && selectedCourt?.image_url"
                ...
                <button @click="showPhotoModal = false" type="button"
                    class="absolute right-4 top-4 flex h-10 w-10 items-center justify-center rounded-full bg-white/15 text-white backdrop-blur-md hover:bg-white/30">
                    <X class="h-5 w-5" />
                </button>
                <div class="flex max-h-[90vh] w-full max-w-4xl flex-col items-center gap-3">
                    <img :src="selectedCourt.image_url" :alt="selectedCourt.name"
                        class="max-h-[80vh] w-auto rounded-2xl object-contain shadow-2xl" />
                    <div class="text-center text-white">
                        <p class="text-lg font-bold">{{ selectedCourt.name }}</p>
                        <p v-if="selectedCourt.description" class="mt-1 max-w-2xl text-sm text-white/80">

...
async function loadSlotsForDate() {
    ...
    try {
        const res = await axios.get('/api/slots/day', { params: { date: createForm.value.date } });
        availableSlots.value = (res.data as TimeSlot[]).filter((s) => s.status === 'available');
    } catch {
...
watch(() => createForm.value.date, loadSlotsForDate);
...
function openCreateModal() {
    createForm.value = { user_id: '', date: '', slot_ids: [], notes: '' };
    availableSlots.value = [];
    createError.value = '';
        ...
        >
            ...
            >
                ...
                <div class="mb-4">
                    ...
                    <div v-if="!createForm.date" class="rounded-xl bg-muted/50 py-6 text-center text-sm text-muted-foreground">

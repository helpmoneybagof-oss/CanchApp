...
import { Edit2, Plus, ShieldCheck, Trash2, X, Goal } from 'lucide-vue-next';
import { ref } from 'vue';
...
function save() {
    ...
    if (editing.value) {
        router.put(`/admin/courts/${editing.value.id}`, payload, {
            onSuccess: () => { toast.success('Cancha actualizada.', '¡Listo!'); showModal.value = false; },
            onError: (e) => { formError.value = Object.values(e)[0] as string; },
            onFinish: () => { saving.value = false; },
        });
    } else {
        router.post('/admin/courts', payload, {
            onSuccess: () => { toast.success('Cancha creada.', '¡Listo!'); showModal.value = false; },
            onError: (e) => { formError.value = Object.values(e)[0] as string; },
            onFinish: () => { saving.value = false; },
        });
    }
